...
class Dashboard(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        con.close()
        run_rows = "".join(f"<tr><td>{escape(human_time(x[0]))}</td><td>{x[3]}</td><td>{x[4]}</td><td>{escape(str(x[6] or ''))}</td></tr>" for x in runs) or "<tr><td colspan='4'>Încă nu există rulări.</td></tr>"
        email_rows = "".join(f"<tr><td>{escape(human_time(x[2]))}</td><td>{escape(str(x[1]))}</td><td>{escape(str(x[0]))}</td></tr><tr class='email-content-row'><td colspan='3'><details><summary>Vezi conținut</summary><div class='mail-preview'>{x[3]}</div></details></td></tr>" for x in emails) or "<tr><td colspan='3'>Încă nu s-a trimis niciun e-mail.</td></tr>"
        page = f"""<!doctype html><html lang='ro'><meta charset='utf-8'><title>Monitor licitații</title><style>
body{{font:15px system-ui,sans-serif;max-width:1100px;margin:40px auto;padding:0 20px;color:#172033}}h1{{margin-bottom:4px}}section{{background:#f7f9fc;border:1px solid #d9e1ed;border-radius:10px;padding:20px;margin:22px 0}}input{{width:min(600px,100%);padding:9px;margin:6px 0}}button{{padding:9px 15px;background:#1769aa;color:#fff;border:0;border-radius:5px}}table{{width:100%;border-collapse:collapse}}th,td{{padding:9px;text-align:left;border-bottom:1px solid #d9e1ed;vertical-align:top}}th{{color:#536174}}small{{color:#536174}}summary{{cursor:pointer;color:#1769aa}}.scroll-area{{max-height:280px;overflow:auto;background:#fff}}.scroll-area th{{position:sticky;top:0;background:#fff}}.email-table{{table-layout:fixed}}.email-table th:nth-child(1){{width:18%}}.email-table th:nth-child(2){{width:30%}}.mail-preview{{box-sizing:border-box;width:100%;margin-top:12px;background:#fff;padding:12px}}.mail-preview table{{width:100%;table-layout:fixed}}.mail-preview td,.mail-preview th{{overflow-wrap:anywhere;word-break:break-word}}.email-content-row td{{padding-top:0;background:#f7f9fc}}</style>
...
<section><h2>Istoric rulări</h2><div class='scroll-area'><table><tr><th>Pornită</th><th>Noi SEAP</th><th>Noi DataDriven</th><th>Detalii</th></tr>{run_rows}</table></div></section>
<section><h2>E-mailuri trimise</h2><table class='email-table'><tr><th>Moment</th><th>Destinatari</th><th>Subiect</th></tr>{email_rows}</table></section></html>"""
        body = page.encode()
