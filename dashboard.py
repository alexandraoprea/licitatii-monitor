...
class Dashboard(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        runs = con.execute("SELECT started_at, completed_at, status, found_count, sent_count, message FROM runs ORDER BY id DESC LIMIT 12").fetchall()
        deliveries = con.execute("SELECT notice_no, title, cpv, recipients, sent_at FROM deliveries ORDER BY id DESC LIMIT 50").fetchall()
        con.close()
        run_rows = "".join(f"<tr><td>{escape(human_time(x[0]))}</td><td>{escape(str(x[2]))}</td><td>{x[3]}</td><td>{x[4]}</td><td>{escape(str(x[5] or ''))}</td></tr>" for x in runs) or "<tr><td colspan='5'>Încă nu există rulări.</td></tr>"
        delivery_rows = "".join(f"<tr><td>{escape(human_time(x[4]))}</td><td>{escape(str(x[0]))}</td><td>{escape(str(x[1]))}</td><td>{escape(str(x[2]))}</td><td>{escape(str(x[3]))}</td></tr>" for x in deliveries) or "<tr><td colspan='5'>Încă nu s-a trimis niciun e-mail.</td></tr>"
        page = f"""<!doctype html><html lang='ro'><meta charset='utf-8'><title>Monitor SEAP</title><style>
...
<section><h2>Istoric rulări</h2><table><tr><th>Pornită</th><th>Stare</th><th>Noi</th><th>Trimise</th><th>Detalii</th></tr>{run_rows}</table></section>
<section><h2>E-mailuri trimise</h2><table><tr><th>Moment</th><th>Anunț</th><th>Obiect</th><th>CPV</th><th>Destinatari</th></tr>{delivery_rows}</table></section></html>"""
        body = page.encode()
